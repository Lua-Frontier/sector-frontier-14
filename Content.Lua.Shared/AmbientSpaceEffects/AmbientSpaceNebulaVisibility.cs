...
public sealed class AmbientSpaceNebulaVisibility
{
    ...
    private SharedMapSystem Map => _map ??= _entManager.System<SharedMapSystem>();
    private TurfSystem Turf => _turf ??= _entManager.System<TurfSystem>();
    ...
    private bool IsBlockedAt(MapId mapId, Vector2 worldPos)
    {
        ...
        {
            var indices = Map.WorldToTile(grid.Owner, grid.Comp, worldPos);

            if (!Map.TryGetTileRef(grid.Owner, grid.Comp, indices, out var tile))
                continue;
    ...
    {
        return !tile.Tile.IsEmpty && !Turf.IsSpace(tile);
    }
}
