// Mono ShipRepair - charge refill cartridges (adapted to SF derived-charges API)
...
public abstract partial class SharedChargesSystem
{
    ...
    private void RefillAmmo(Entity<LimitedChargesAmmoComponent> ent, EntityUid target, EntityUid user)
    {
        ...
        var current = GetCurrentCharges((target, charges, null));
        var count = Math.Min(charges.MaxCharges - current, GetAmmoCharges(ent));
        if (count <= 0)
        {
