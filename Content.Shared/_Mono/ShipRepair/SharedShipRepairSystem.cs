...
public abstract partial class SharedShipRepairSystem : EntitySystem
{
    ...
    /// <summary>
    /// Tools without LimitedCharges (e.g. admin SRD) are treated as having infinite charges.
    /// </summary>
    protected bool HasInsufficientCharges(EntityUid uid, int requiredCharges)
    {
        if (!HasComp<Content.Shared.Charges.Components.LimitedChargesComponent>(uid))
            return false;

        return !_charges.HasCharges(uid, requiredCharges);
    }

    protected void UseCharges(EntityUid uid, int chargesUsed)
    {
        if (!HasComp<Content.Shared.Charges.Components.LimitedChargesComponent>(uid))
            return;

        _charges.TryUseCharges(uid, chargesUsed);
    }
}
