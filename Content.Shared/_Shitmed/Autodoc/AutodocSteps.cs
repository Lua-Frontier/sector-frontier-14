...
public sealed partial class SurgeryAutodocStep : IAutodocStep
{
    ...
    bool IAutodocStep.Run(Entity<AutodocComponent, HandsComponent> ent, SharedAutodocSystem autodoc)
    {
        ...
        if (autodoc.IsTendWoundsSurgery(Surgery) && !autodoc.HasTendWoundsDamage(patient, part, Surgery))
        {
            autodoc.Say(ent, Loc.GetString("autodoc-error-tend-wounds-no-damage"));
            return true;
        }
