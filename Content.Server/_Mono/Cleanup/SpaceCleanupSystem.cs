...
public sealed class SpaceCleanupSystem : BaseCleanupSystem<PhysicsComponent>
{
    ...
    private EntityQuery<SectorCelestialBodyComponent> _celestialQuery;
    private EntityQuery<SingularityComponent> _singularityQuery;
    private EntityQuery<TeslaEnergyBallComponent> _teslaQuery;
    ...
    public override void Initialize()
    {
        ...
        _celestialQuery = GetEntityQuery<SectorCelestialBodyComponent>();
        _singularityQuery = GetEntityQuery<SingularityComponent>();
        _teslaQuery = GetEntityQuery<TeslaEnergyBallComponent>();
    ...
    private bool ShouldEntityCleanup(EntityUid uid, float aggression)
    {
        ...
        return !_gridQuery.HasComp(uid)
            ...
            && !_immuneQuery.HasComp(uid) // handled by GridCleanupSystem / prototype CleanupImmune
            && !_celestialQuery.HasComp(uid) // permanent sector landmarks (stars/BHs)
            && !_singularityQuery.HasComp(uid) // gravitational singularity
            && !_teslaQuery.HasComp(uid) // Tesla energy ball (not mini-balls)
