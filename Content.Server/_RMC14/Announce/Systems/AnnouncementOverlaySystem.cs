...
public sealed partial class AnnouncementOverlaySystem : EntitySystem
{
    ...
    public uint EnsureSpeakerPvs(EntityUid? speakerUid, Filter filter)
    {
        if (speakerUid is not { } speaker || !Exists(speaker))
            return 0;
        ...
        Timer.Spawn(
            PvsFallbackTimeout,
            () => CompleteOverride(overrideId),
            tracker.FallbackCancellation.Token);
        return overrideId;
    }
