...
namespace Content.Lua.Server.Company;

public sealed class FactionOwnedStationSystem : EntitySystem, IFactionOwnedStationSystem
{
    ...
    public bool TryGetOriginalOwner(EntityUid station, out string? companyId)
        => TryGetOriginalOwner(station, out companyId, null);

    public bool TryGetOriginalOwner(EntityUid station, out string? companyId, FactionOwnedStationComponent? ownedStation)
    {
        companyId = null;
        if (!Resolve(station, ref ownedStation, false))
            return false;
    ...
    public void SetOwner(EntityUid station, string? companyId)
        => SetOwner(station, companyId, null);

    public void SetOwner(EntityUid station, string? companyId, FactionOwnedStationComponent? ownedStation)
    {
