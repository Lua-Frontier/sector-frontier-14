...
using Content.Lua.Server.SpaceHazards;
using Content.Lua.Server.Starmap.Components;
...
namespace Content.Lua.Server.Starmap.Systems
{
    public sealed class SimpleStarmapSystem : EntitySystem, ISimpleStarmapSystem
    {
        ...
        public void WarpToStar(EntityUid consoleUid, Star star, EntityUid? actor = null)
        {
            ...
            if (!_mapManager.TryGetMap(star.Map, out var mapUid))
            ...
            for (var attempt = 0; attempt < SectorArrivalAttempts; attempt++)
            {
                ...
                if (NebulaEnvironmentSystem.IsFtlBlockedByFields(candidate, _ftlBlockingFields))
                    continue;
                targetPos = candidate;
                targetCoordinates = new EntityCoordinates(mapUid.Value, candidate);
                foundSafeSpot = true;
                break;
            }
        ...
        private void OnFtlCompleted(ref FTLCompletedEvent ev)
        {
            ...
            if (!_mapManager.TryGetMap(transit.TargetMap, out var mapUid))
                return;
            var targetCoords = new EntityCoordinates(mapUid.Value, transit.TargetPosition);
            ...
            if (NebulaEnvironmentSystem.IsFtlBlockedByFields(arrived, _ftlBlockingFields))
            {
                ...
                {
                    _shuttleSystem.TryFTLProximity((shuttle, xform), new EntityCoordinates(mapUid.Value, transit.TargetPosition));
                }
                else
                {
                    for (var attempt = 0; attempt < SectorArrivalAttempts; attempt++)
                    {
                        ...
                        if (NebulaEnvironmentSystem.IsFtlBlockedByFields(candidate, _ftlBlockingFields))
                            continue;
                        _shuttleSystem.TryFTLProximity((shuttle, xform), new EntityCoordinates(mapUid.Value, candidate));
                        break;
                    }
                }
            }
