...
namespace Content.Lua.Server.Horror;
...
public sealed partial class LumenAversionSystem : EntitySystem
{
    ...
    private bool IsDirectional(Entity<PointLightComponent> light, TransformComponent srcXform)
    {
        return light.Comp.LightMask is { } maskId
            && _prototype.TryIndex(maskId, out LightMaskPrototype? mask)
            && mask.MaskPath.ToString().EndsWith("cone.png", StringComparison.Ordinal);
    }
