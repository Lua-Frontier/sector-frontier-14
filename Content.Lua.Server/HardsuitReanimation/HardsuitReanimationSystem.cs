...
namespace Content.Lua.Server.HardsuitReanimation;
...
public sealed class HardsuitReanimationSystem : EntitySystem
{
    ...
    private void TeleportToOrigin(EntityUid wearer, HardsuitReanimationComponent comp)
    {
        ...
        if (!_mapManager.TryGetMap(savedCoordinates.MapId, out var mapUid))
        {
            var noMapMsg = Loc.GetString("hardsuit-reanimation-no-saved-coordinates");
            _popup.PopupEntity(noMapMsg, wearer, wearer);
            return;
        }

        var transform = Transform(wearer);
        Spawn(comp.EmpPulseEffect, transform.Coordinates);
        transform.Coordinates = EntityCoordinates.FromMap(mapUid.Value, savedCoordinates, _transform, EntityManager);
    ...
    private void SaveTeleportCoordinates(EntityUid hardsuit, EntityUid user, HardsuitReanimationComponent comp)
    {
        ...
        var mapEntity = _mapManager.GetMapOrInvalid(coords.MapId);
        if (mapEntity.IsValid() &&
            TryComp<MetaDataComponent>(mapEntity, out var mapMeta) &&
