...
using Content.Shared.EntityEffects.Effects; // Lua
...
public sealed partial class GuideReagentEmbed : BoxContainer, IDocumentTag, ISearchableControl, IPrototypeRepresentationControl
{
    ...
    private readonly SpriteSystem _sprite; // Lua
    ...
    public GuideReagentEmbed()
    {
        ...
        _sprite = _systemManager.GetEntitySystem<SpriteSystem>(); // Lua
    ...
    private void GenerateControl(ReagentPrototype reagent)
    {
        ...
        AlcoholIcon.Visible = true;
        var alcohol = HasReagent(reagent, "Ethanol");
        AlcoholIcon.TextureNormal = alcohol
        ? _sprite.Frame0(new SpriteSpecifier.Rsi(new ResPath("Objects/Consumable/Drinks/cosmopolitan.rsi"), "icon"))
        : _sprite.Frame0(new SpriteSpecifier.Rsi(new ResPath("Objects/Consumable/Drinks/kiraspecial.rsi"), "icon"));
        AlcoholIcon.ToolTip = alcohol
        ? Loc.GetString("guidebook-reagent-alcohol-tooltip")
        : Loc.GetString("guidebook-reagent-alcohol-free-tooltip");

        if (HasReagent(reagent, "Theobromine"))
        {
            TheobromineIcon.Visible = true;
            TheobromineIcon.TextureNormal = _sprite.Frame0(new SpriteSpecifier.Rsi(new ResPath("Objects/Specific/Hydroponics/tea_plant.rsi"), "produce"));
        }
    }

    private static bool HasReagent(ReagentPrototype proto, string targetReagent)
    {
        if (proto.ID == targetReagent)
        {
            return true;
        }
        ...
        foreach (var effectsEntry in proto.Metabolisms.Values)
        {
            foreach (var effect in effectsEntry.Effects)
            {
                if (effect is AdjustReagent adjust
                    && adjust.Reagent == targetReagent
                    && adjust.Amount > 0f)
                {
                    return true;
                }
