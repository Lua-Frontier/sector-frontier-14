...
public sealed partial class PlayerTabEntry : PanelContainer
{
        ...
        AdminPlayerTabSymbolOption symbolSetting)
    {
        ...
        BalanceLabel.Text = player.Balance == int.MinValue
            ? Loc.GetString("player-tab-balance-none")
            : BankSystemExtensions.ToSpesoString(player.Balance);
        ReputationLabel.Text = player.ReputationString; // Frontier
        CompanyLabel.Text = string.IsNullOrWhiteSpace(player.Company)
            ? Loc.GetString("player-tab-company-none")
            : player.Company;
        DonatLabel.Text = FormatDonat(player.Donat);
    }

    private static string FormatDonat(string donat)
    {
        if (string.IsNullOrWhiteSpace(donat))
            return Loc.GetString("player-tab-donat-none");

        var parts = donat.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        if (parts.Length == 0)
            return Loc.GetString("player-tab-donat-none");

        return string.Join(", ", parts.Select(DonorGroups.GetTierDisplayName));
    }
}
