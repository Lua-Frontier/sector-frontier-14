...
public sealed partial class GunPredictionSystem : SharedGunPredictionSystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (serverProjectiles.MoveNext(out _, out var serverProjectile, out var sprite))
        {
        ...
        while (predictedQuery.MoveNext(out _, out var hitComp, out var sprite, out var xform))
        {
            var origin = hitComp.Origin;
            var coordinates = xform.Coordinates;
            if (!origin.TryDistance(EntityManager, _transform, coordinates, out var distance) ||
                distance >= hitComp.Distance)
            {
