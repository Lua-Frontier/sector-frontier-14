...
public sealed class AnnouncementOverlayWidget : UIWidget
{
    ...
    public void Reflow()
    {
        var active = _announcements
            .Where(widget => widget.ActiveAnnouncement != null)
            .ToArray();
        if (active.Length == 0)
            return;

        var sizes = new Vector2[active.Length];
        var stackWidth = 0f;
        var stackHeight = 0f;

        for (var i = 0; i < active.Length; i++)
        {
            var layout = active[i].ResolveLayout();
            sizes[i] = layout.Size;
            stackWidth = MathF.Max(stackWidth, layout.Size.X);
            stackHeight += layout.Size.Y;
        }

        stackHeight += StackSeparation * (active.Length - 1);
        var stackSize = new Vector2(stackWidth, stackHeight);
        var anchorLayout = active[0].ResolveLayout(stackSize);
        var alignment = active[0].ResolveStackAlignment();
        var positions = CalculateStackPositions(anchorLayout.Position, stackWidth, sizes, alignment, StackSeparation);

        for (var i = 0; i < active.Length; i++)
        {
            active[i].ApplyManagedLayout(positions[i], sizes[i]);
        }
    }
