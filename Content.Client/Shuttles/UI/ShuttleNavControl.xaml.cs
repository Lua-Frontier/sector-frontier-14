...
public partial class ShuttleNavControl : BaseShuttleControl // Mono
{
    ...
    public ShuttleNavControl(float minRange, float maxRange, float range) : base(minRange, maxRange, range) // Mono
    {
        ...
        _cipherFont = IoCManager.Resolve<IResourceCache>().GetFont("/Fonts/Drakkhen/drakkhen_beta.ttf", 12); // Lua Decrypt mod
    ...
    protected override void Draw(DrawingHandleScreen handle)
    {
        ...
        foreach (var grid in _grids)
        {
            ...
            var hideLabelShuttle = iff != null && (iff.Flags & IFFFlags.HideLabelShuttle) != 0x0; // Lua Decrypt mod
            ...
            if (hideLabelShuttle)
            {
                ...
                { labelName = string.Concat(decrypt.Revealed, CipherStart, decrypt.Cipher, CipherEnd); cipherName = true; }
                else
            ...
            // Lua decrypt mod start
            bool allowBlip = !hideLabel || hideLabelShuttle;
            if (ShowIFF && allowBlip) NFAddBlipToList(blipDataList, isOutsideRadarCircle, uiPosition, uiXCentre, uiYCentre, labelColor);
            if (shouldDrawIFF)
            { // Lua decrypt mod end
